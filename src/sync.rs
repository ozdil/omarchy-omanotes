...
pub fn export_encrypted_envelope(state: &AppState, password: &str) -> Result<PathBuf, String> {
    ...
    let salt_bytes = if state.salt.is_empty() {
        ...
        let decoded = hex::decode(&state.salt).map_err(|e| format!("Invalid stored salt: {}", e))?;
        if decoded.len() != 16 {
    ...
    let plaintext_bytes = serde_json::to_vec(&state.notes).map_err(|e| format!("Serialization error: {}", e))?;

    let envelope = crypto::encrypt_payload(&plaintext_bytes, &key, &salt_bytes)?;
    let envelope_bytes = serde_json::to_vec_pretty(&envelope).map_err(|e| format!("Envelope serialize error: {}", e))?;
...
#[allow(dead_code)]
pub fn import_encrypted_envelope(enc_path: &PathBuf, password: &str) -> Result<Vec<storage::Note>, String> {
    let bytes = storage::safe_read_0600(enc_path)?;
    let envelope: EncryptedEnvelope = serde_json::from_slice(&bytes).map_err(|e| format!("Invalid envelope JSON: {}", e))?;
...
pub fn sync_cloud(state: &mut AppState, password: &str) -> SyncResult {
    ...
    let enc_file_str = match enc_file.to_str() {
        Some(s) => s,
        None => return SyncResult { success: false, message: "Invalid path UTF-8".to_string() },
    };
    ...
    match provider.as_str() {
        "rclone" => {
            if state.cloud.rclone_remote.trim().is_empty() {
                return SyncResult {
                    success: false,
                    message: "Rclone remote path is not configured (e.g., gdrive:OmarchyNotes)".to_string(),
                };
            }

            let remote_dest = format!("{}/notes.enc", state.cloud.rclone_remote.trim_end_matches('/'));
            ...
            if out.is_some() {
                ...
                state.cloud.last_sync_msg = format!("Encrypted notes synced to {}", state.cloud.rclone_remote);
                let _ = storage::save_app_state(state);
                SyncResult {
                    success: true,
                    message: format!("Successfully synced encrypted notes to {}", state.cloud.rclone_remote),
                }
            } else {
                state.cloud.last_sync_status = "error".to_string();
        ...
        "git" => {
            ...
            let repo_str = match repo_path.to_str() {
                Some(s) => s,
                None => return SyncResult { success: false, message: "Invalid git path UTF-8".to_string() },
            };
            ...
            let _ = run_cmd_bounded("/usr/bin/git", &add_args, &[("GIT_TERMINAL_PROMPT", "0")], deadline, 32768);

            // git -C <repo> commit -m "sync: e2ee encrypted notes update"
            let commit_args = ["-C", repo_str, "commit", "-m", "sync: e2ee encrypted notes update", "--", "notes.enc"];
            let _ = run_cmd_bounded("/usr/bin/git", &commit_args, &[("GIT_TERMINAL_PROMPT", "0")], deadline, 32768);

            // git -C <repo> push
            let push_args = ["-C", repo_str, "push"];
            let push_out = run_cmd_bounded("/usr/bin/git", &push_args, &[("GIT_TERMINAL_PROMPT", "0")], deadline, 32768);

            if push_out.is_some() {
                state.cloud.last_synced_at = now_secs();
                state.cloud.last_sync_status = "synced".to_string();
                state.cloud.last_sync_msg = "Pushed encrypted notes to Git".to_string();
                let _ = storage::save_app_state(state);
                SyncResult {
                    success: true,
                    message: "Pushed encrypted notes to Git repository".to_string(),
                }
            } else {
                state.cloud.last_sync_status = "error".to_string();
                state.cloud.last_sync_msg = "Git push failed or timed out".to_string();
                let _ = storage::save_app_state(state);
                SyncResult {
                    success: false,
                    message: "Git push failed. Ensure SSH keys or tokens are valid.".to_string(),
                }
            }
        }
        _ => SyncResult {
